...
export default defineComponent({
  ...
  mounted () {
    const getMetaData = (script) => {
      const meta = script.match(
        /\/\/ ==UserScript==\n(\/\/ @.+\n)+\/\/ ==\/UserScript==/g,
      )?.[0];
      ...
      for (const [, line] of meta.matchAll(/\/\/ @(.+)\n/g)) {
        const [, k, v] = line.match(/(\S+)\s*(.+)?/);

        const val = metaData[k];
        if (val) {
          if (typeof val === 'string') metaData[k] = [val, v];
          else val.push(v);
        } else {
          metaData[k] = v ?? '';
        }
    ...
    (async () => {
      // const url = `http://localhost:2405/index.js?${Date.now()}`;
      const url = 'https://update.sleazyfork.org/scripts/374903/ComicRead.user.js';
      const res = await fetch(url);
      const script = await res.text();
      const metaData = getMetaData(script);

      const resource = {};
      for (const line of metaData.resource) {
        const [name, url] = line.split(/\s+/);
        const res = await fetch(url);
        resource[name] = await res.text();
      }

      window.GM_getResourceText = (name) => resource[name] ?? '';

      window.GM_xmlhttpRequest = async (details) => {
        const res = await fetch(url, {
          method: 'GET',
          headers,
      ...
      window.GM = {
        ...
        info: { script: { version: metaData.version } }
      };


      window.GM.getValue = (key, val) => {
      ...
      window.GM.setValue = (key, val) =>
        localStorage.setItem(key, typeof val === 'string' ? val : JSON.stringify(val));
      window.GM.deleteValue = (...args) => localStorage.removeItem(...args);
      window.GM.listValues = () => Object.keys(localStorage);

      const menuCommand = {};
      window.GM.registerMenuCommand = (name, func) => {
        menuCommand[name] = func;
      };
      window.GM.unregisterMenuCommand = (name) => {
        delete menuCommand[name];
      };
