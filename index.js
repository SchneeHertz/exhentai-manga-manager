...
const {createAndPopulateHashTable, createAndPopulateTitleTable, sanitizeTitle} = require('./modules/database_meta');
            ...
            for (let i = 0; i < bookListLength; i++) {
                ...
                if (book.status !== 'tagged') {
                    ...
                    if (metadata === undefined) {
                        ...
                        // cover hash is extracted and indexed from the thumb col; but since 2024-07 the col has stop storing hash
                        metadata = await db.get('SELECT t.* FROM gallery t JOIN coverhash h USING (gid) WHERE h.hash = ?',
                            [book.coverHash]);
...
ipcMain.handle('import-sqlite-filename', async (event, bookList) => {
    // match by filename
    const result = await dialog.showOpenDialog(mainWindow, {
        properties: ['openFile'],
        filters: [{name: 'SQLite', extensions: ['sqlite']}]
    ...
    if (!result.canceled) {
        ...
        try {
            const re = /'/g
            const bookListLength = bookList.length
            await createAndPopulateTitleTable(db, mainWindow)

            for (let i = 0; i < bookListLength; i++) {
                const book = bookList[i]
                if (book.status !== 'tagged') {
                    // remove extension and all empty spaces
                    const booktitle = sanitizeTitle(book.title)
                    const metadata = await db.get("SELECT * FROM gallery WHERE gid IN (SELECT gid FROM filename WHERE title = ?)",
                        [booktitle])
